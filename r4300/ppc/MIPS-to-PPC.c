...
static int branch(int offset, condition cond, int link, int likely){
	...
	if(is_j_out(offset, 0)){
		...
		set_next_dst(ppc);
		GEN_STW(ppc, 3, SDAREL(last_addr), 13);
		set_next_dst(ppc);
...
static int SLTI(MIPS_instr mips){
	...
	// r2 = immed (sign extended)
	GEN_ADDI(ppc, 2, 0, MIPS_GET_IMMED(mips));
	set_next_dst(ppc);
	// carry = rs < immed ? 0 : 1 (unsigned)
	GEN_SUBFC(ppc, 0, 2, rs);
	set_next_dst(ppc);
	// rt = ~(rs ^ immed)
	GEN_EQV(ppc, rt, 2, rs);
	set_next_dst(ppc);
...
static int LWL(MIPS_instr mips){
	...
	genCallDynaMem(MEM_LWL, base, MIPS_GET_IMMED(mips));
...
static int LWR(MIPS_instr mips){
	...
	genCallDynaMem(MEM_LWR, base, MIPS_GET_IMMED(mips));
...
static int SWL(MIPS_instr mips){
	...
	genCallDynaMem(MEM_SWL, base, MIPS_GET_IMMED(mips));
...
static int SWR(MIPS_instr mips){
	...
	genCallDynaMem(MEM_SWR, base, MIPS_GET_IMMED(mips));
...
static int CFC1(MIPS_instr mips){
	...
	if(MIPS_GET_FS(mips) == 31){
		...
		GEN_LWZ(ppc, rt, SDAREL(FCR31), 13);
		set_next_dst(ppc);
...
static int CTC1(MIPS_instr mips){
	...
	if(MIPS_GET_FS(mips) == 31){
		...
		GEN_STW(ppc, rt, SDAREL(FCR31), 13);
		set_next_dst(ppc);
	}
...
static int CVT_W_FP(MIPS_instr mips, int dbl){
	...
	GEN_LFD(ppc, 0, SDAREL(FCR31-4), 13);
	set_next_dst(ppc);
...
static void genUpdateCount(int checkCount){
	...
	if(checkCount){
		...
		GEN_LWZ(ppc, 2, SDAREL(next_interupt), 13);
		set_next_dst(ppc);
	}
