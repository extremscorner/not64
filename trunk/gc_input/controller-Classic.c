...
static int _GetKeys(int Control, BUTTONS * Keys )
{
	...
	c->U_DPAD       = (b & WPAD_CLASSIC_BUTTON_UP)    ? 1 : 0;
	c->START_BUTTON = (b & WPAD_CLASSIC_BUTTON_PLUS)  ? 1 : 0;
	c->B_BUTTON     = (b & WPAD_CLASSIC_BUTTON_B)     ? 1 : 0;
	c->A_BUTTON     = (b & WPAD_CLASSIC_BUTTON_A)     ? 1 : 0;

	c->Z_TRIG       = (b & WPAD_CLASSIC_BUTTON_ZR)     ? 1 : 0;
